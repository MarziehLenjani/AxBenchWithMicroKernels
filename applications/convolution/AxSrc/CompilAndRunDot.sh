...
#echo  DotProduct  iNumberOfBits	AverageRelativeError	MaximumAbsoluteError
if [ $Sampling == 1 ] ; then
for  (( i=0; i<=$numOfRun; i=i+$step )) 
do
# echo -n $i
 ./convolutionAx 512 512 512 512 1 1 512 512 1 1  ${outputdir}/AxOutOrg ${i} 6 1.5 16 1  ${FthArg} ${SthArg} ${TthArg} ${FOthArg}
...
do
 #echo -n $i
 ./convolutionAx 512 512 512 512 1 1 512 512 1 1  ${outputdir}/AxOutOrg ${i} 6 1.5 16 1
 python ../scripts/qos.py ${outputdir}/AxOutOrg_0.txt ${outputdir}/AxOutOrg_${i}.txt
done
fi
:<<'END'
