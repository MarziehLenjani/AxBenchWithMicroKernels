...
#		print "--------------",diff1 , diff2, origItem1 ,origItem2, nnItem1, nnItem2 
	...
	if(math.fabs(diff1)<MinAbsError and not math.isnan(diff1)):
		MinAbsError=math.fabs(diff1)
	if(math.fabs(diff2)<MinAbsError and not math.isnan(diff2)):
		MinAbsError=math.fabs(diff2)
pass;
...
print "  %1.8f" % relError + "    "+ "  %1.8f" % (MaxAbsoulteErrorVar1)+ "  %1.8f" % (relAbsError)+ " %1.8f" % (MinAbsError)
