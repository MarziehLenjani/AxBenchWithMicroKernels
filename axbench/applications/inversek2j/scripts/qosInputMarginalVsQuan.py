...
for i in range(2,len(origLines)):
	...
	mantiss2, exponetnt2= math.frexp(origItem2)
	if(math.fabs(origItem1)>MaxItem1):
		MaxItem1=origItem1
	if(math.fabs(origItem2)>MaxItem2):
		MaxItem2=origItem2
	if(exponetnt1>MaxExponentItem1):
		MaxExponentItem1=exponetnt1
	if(exponetnt2>MaxExponentItem2):
		MaxExponentItem2=exponetnt2
pass;
...
for i in range(1,32):
	print "	%d " % i+"    "+"  %f" % (math.pow(2,MaxExponentItem1)*math.pow(2,-23)*(math.pow(2,32-i)-1)) + "    "+ "  %f" % ((math.pow(2,MaxExponentItem2)*math.pow(2,-23)*(math.pow(2,32-i)-1))) +"  %f" % (MaxItem1/math.pow(2,i+1)) + "    "+ "  %f" % (MaxItem2/math.pow(2,i)) 
pass;