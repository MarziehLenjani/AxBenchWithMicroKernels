...
for i in range(len(origLines)):
	...
	nnReal 		= float(nnLine.split(" ")[0])
 	...
 	absError += e
	AbsoluteError+=nominator
pass;

print bcolors.FAIL	+ nnFilename +"*** Error:    %1.8f" % (absError/float(len(origLines))) + "    "+ bcolors.ENDC
print bcolors.FAIL	+ nnFilename +"*** ABS:    %20.8f" % (AbsoluteError/float(len(origLines))) + "    "+ bcolors.ENDC
